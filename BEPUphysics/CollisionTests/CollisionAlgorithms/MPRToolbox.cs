...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public static class MPRToolbox
    {
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref Vector3 direction, out FP t, out Vector3 normal)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon * F64.C0p01)
                    Vector3.Divide(ref direction, FP.Sqrt(rayLengthSquared), out normal);
                else
                    normal = new Vector3();
            ...
            while (true)
            {
                ...
                if (supportDot - dot < surfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    {
                        Vector3.Divide(ref n, FP.Sqrt(lengthSquared), out normal);
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref Vector3 direction, out FP t, out Vector3 normal, out Vector3 position)
        {
            // Local surface cast is very similar to regular MPR.  However, instead of starting at an interior point and targeting the origin,
            // the ray starts at the origin (a point known to be in both shape and shapeB), and just goes towards the direction until the surface
            // is found.  The portal (v1, v2, v3) at termination defines the surface normal, and the distance from the origin to the portal along the direction is used as the 't' result.
        ...
        static bool VerifySimplex(ref Vector3 v0, ref Vector3 v1, ref Vector3 v2, ref Vector3 v3, ref Vector3 direction)
        {
            ...
            FP planeProduct1 = Vector3.Dot(cross, direction);
            //v3, v0, v2
            cross = Vector3.Cross(v0 - v3, v2 - v3);
            FP planeProduct2 = Vector3.Dot(cross, direction);
            //v2, v0, v1
            cross = Vector3.Cross(v0 - v2, v1 - v2);
            FP planeProduct3 = Vector3.Dot(cross, direction);
        ...
        public static bool GetContact(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB, ref Vector3 penetrationAxis, out ContactData contact)
        {
            ...
            if (MPRToolbox.AreLocalShapesOverlapping(shapeA, shapeB, ref localTransformB))
            {
                ...
                {
                    Vector3.Divide(ref penetrationAxis, FP.Sqrt(lengthSquared), out rayCastDirection);// (Vector3.Normalize(localDirection) + Vector3.Normalize(collidableB.worldTransform.Position - collidableA.worldTransform.Position)) / 2;
                ...
                if (lengthSquared > Toolbox.Epsilon)
                {
                    Vector3.Divide(ref localTransformB.Position, FP.Sqrt(lengthSquared), out rayCastDirection);
        ...
        public static void RefinePenetration(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, FP initialDepth, ref Vector3 initialNormal, out FP penetrationDepth, out Vector3 refinedNormal, out Vector3 position)
        {
            //The local casting can optionally continue.  Eventually, it will converge to the local minimum.
        ...
        private static bool LocalSweepCast(ConvexShape shape, ConvexShape shapeB, FP sweepLength, FP rayLengthSquared, ref Vector3 localDirection, ref Vector3 sweep, ref RigidTransform localTransformB, out RayHit hit)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon * F64.C0p01)
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon * F64.C0p01)
                    Vector3.Divide(ref localDirection, FP.Sqrt(rayLengthSquared), out hit.Normal);
                else
                    hit.Normal = new Vector3();
            ...
            while (true)
            {
                ...
                if (supportDot - dot < rayCastSurfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    {
                        Vector3.Divide(ref n, FP.Sqrt(lengthSquared), out hit.Normal);
        ...
        internal static void GetLocalPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref Vector3 minkowskiPosition, out Vector3 position)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                FP dot = Vector3.Dot(v1 - minkowskiPosition, rayDirection);
                ...
                //Origin is inside.
                //Compute barycentric coordinates along simplex (segment).
                FP dotv0 = Vector3.Dot(v0 - minkowskiPosition, rayDirection);
                //Dot > 0, so dotv0 starts out negative.
        ...
        /// <returns>Whether or not the swept shapes hit each other.</returns>
        public static bool RayCast(Ray ray, FP maximumLength, ConvexShape shape, ref RigidTransform transform, out RayHit hit)
        {
        ...
        private static bool LocalSweepCast(ConvexShape shape, FP sweepLength, FP rayLengthSquared, ref Vector3 localDirection, ref Vector3 sweep, ref Vector3 rayOrigin, out RayHit hit)
        {
            //By now, the ray is known to be within the swept shape and facing the right direction for a normal raycast.
