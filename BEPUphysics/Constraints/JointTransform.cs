...
namespace BEPUphysics.Constraints
{
    ...
    public class JointBasis3D
    {
        ...
        {
            if (FP.Abs(Vector3.Dot(primaryAxis, xAxis)) > Toolbox.BigEpsilon ||
				FP.Abs(Vector3.Dot(primaryAxis, yAxis)) > Toolbox.BigEpsilon ||
				FP.Abs(Vector3.Dot(xAxis, yAxis)) > Toolbox.BigEpsilon)
        ...
        {
            if (FP.Abs(Vector3.Dot(matrix.Backward, matrix.Right)) > Toolbox.BigEpsilon ||
				FP.Abs(Vector3.Dot(matrix.Backward, matrix.Up)) > Toolbox.BigEpsilon ||
				FP.Abs(Vector3.Dot(matrix.Right, matrix.Up)) > Toolbox.BigEpsilon)
                throw new ArgumentException("The axes provided to the joint transform do not form an orthonormal basis.  Ensure that each axis is perpendicular to the other two.");
